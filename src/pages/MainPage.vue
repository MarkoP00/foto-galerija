...
async function getImages() {
  const response = await fetch(
    "https://aleksandar-photo-default-rtdb.europe-west1.firebasedatabase.app/images.json"
  );
...
function deleteSingleImage(id) {
  sortArray.value = sortArray.value.filter((image) => image.id !== id);
  imagesData.value = imagesData.value.filter((image) => image.id !== id);
}
